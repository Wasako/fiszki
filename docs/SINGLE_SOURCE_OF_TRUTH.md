...
- **Układ:** przed odpowiedzią — w **`.quiz-prompt-slot`** (stała wysokość `clamp`, bez przesuwania przycisków opcji) etykieta działu i **`.quiz-question-title`**; po wyborze — **`.quiz-flip-face`** z pełnym wzorem i legendą (przewijanie w slocie przy długiej treści). Główny wzór (**`.quiz-flip-formula`**) ma wyraźnie większą bazę (**~1.55rem**) i KaTeX **~1.65em**; legenda w tej fiszce (**`.quiz-flip-face .symbol-legend*`** — wiersze / KaTeX) lekko większa niż domyślna **`.symbol-legend-row`**. Cztery przyciski z wariantami LaTeXu pod spodem (większy KaTeX w opcjach).
